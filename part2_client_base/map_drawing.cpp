...
void draw_route(){
  ...
  for(int i = 0; i < shared.num_waypoints-1; ++i){
    int startX = longitude_to_x(shared.map_number,shared.waypoints[i].lon);
    int startY = latitude_to_y(shared.map_number,shared.waypoints[i].lat);
    int endX = longitude_to_x(shared.map_number,shared.waypoints[i+1].lon);
    int endY = latitude_to_y(shared.map_number,shared.waypoints[i+1].lat);
